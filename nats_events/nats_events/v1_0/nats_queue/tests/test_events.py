...
SETTINGS = {
    "plugin_config": {
        "nats_queue": {
            "connection": {"connection_url": "nats://localhost:4222"},
            "inbound": {
...
TEST_PAYLOAD_BYTES = orjson.dumps(TEST_PAYLOAD_DICT)


class TestNATSEvents(IsolatedAsyncioTestCase):
    ...
    async def test_on_startup_x(self):
        ...
        with patch.object(
            NATS,
            "connect",
            AsyncMock(side_effect=ErrNoServers),
        ):
